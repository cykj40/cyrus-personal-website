...
import { RESUME_PDF_PATH } from '@/components/resume/resume-pdf';

// The resume modal renders the full CV (~17 kB minified). Nobody sees it until
// they click "View my resume", so it is fetched on that click instead of riding
// along in this page's chunk. `prefetchResumeModal` warms it on hover/focus, so
// by the time the click lands the chunk is usually already in cache.
const importResumeModal = () => import('@/components/resume/ResumeModal');
const ResumeModal = lazy(() => importResumeModal().then((m) => ({ default: m.ResumeModal })));

let resumeModalPrefetched = false;
const prefetchResumeModal = () => {
  if (resumeModalPrefetched) return;
  resumeModalPrefetched = true;
  void importResumeModal();
};
        ...
        >
          ...
          <motion.div variants={fadeUp} className="flex flex-wrap gap-3">
            <Button
              ...
              onMouseEnter={prefetchResumeModal}
              onFocus={prefetchResumeModal}
            >
              View my resume
            </Button>
            <a
...
export function EngineeringContent({ includeAbout = false }: EngineeringContentProps) {
  ...
  // Stays true once opened so the modal keeps its AnimatePresence exit animation
  // on close; before the first open it is never mounted, so the chunk is never
  // requested.
